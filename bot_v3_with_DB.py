...
async def send_random_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    random_message = str(random_message[0])
    ...
    while True:
        ...
        try:
            ...
            if(ex.message == "Chat not found"):
                ...
                # Save the scheduled jobs to a DB
                write_to_DB()
            else:
...
def read_from_DB():
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            scheduled_jobs_from_DB = scheduled_jobs_raw[0]
...
def write_to_DB():
    ...
    try:
        ...
        new_value = str(scheduled_jobs).replace('\'', '"')  # This is your variable
        row_id = 1  # This is the ID of the row you want to update

        # Define the SQL query with placeholders for the variable
        query = text("UPDATE public.schedule SET schedule = :new_value")

        # Execute the query with the variable value
        session.execute(query, {"new_value": new_value, "row_id": row_id})
